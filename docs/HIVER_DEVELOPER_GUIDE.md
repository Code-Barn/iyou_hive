...
*Last Updated: 2026-05-15* | *Build: index-I83D9tTN.js / index-BY2PQJFe.css*
...
Unauthenticated users hitting any page are redirected through the OIDC lifecycle:

1. **React SPA or Django page** → `react_app_view` → unauthenticated → `redirect('oidc_authentication_init')`
2. **OIDC Init** → Redirects to iyou_idp (`:8000/openid/authorize/`)
3. **IdP Auth** → User authenticates with their Sovereign Passport (DID)
4. **OIDC Callback** → `:8003/oidc/callback/` → `MyOIDCAuthenticationBackend` maps `sub` claim (DID) → Django User
5. **Post-Login** → `LOGIN_REDIRECT_URL = "/"` → `react_app_view` renders React SPA
...
| mozilla-django-oidc | 5.0.2 | OIDC Relying Party (Sovereign Auth) |
...
|--------|-------------|---------|
| iyou_idp (:8000) | OIDC Provider | Sovereign Identity Provider |
| Desktop Vault (:9001) | Tauri Bridge | Cryptographic signing (sign_evidence) |
| Ollama | LLM Client | Local LLM Inference |
| Google Gemini | LLM Client | Cloud LLM Access |
| Rust-DID | FFI via ctypes | Decentralized Identity (legacy) |
...
└── QueryClientProvider (TanStack React Query)
    └── Layout.tsx (3-Panel Dashboard)
        ├── SovereignHeader.tsx (Mesh nav bar, :9001 probe, DID badge)
        ├── LEFT: Timeline Panel
...
|--------|----------|-------------|
| GET | `/oidc/authenticate/` | OIDC login (redirects to IdP) |
| GET | `/oidc/callback/` | OIDC callback (IdP → user created) |
| POST | `/oidc/logout/` | OIDC logout |
| GET | `/accounts/login/` | Legacy login (deprecated) |
| POST | `/accounts/did/login/` | DID login (legacy Rust-DID) |
...
#### EvidenceSignature (Vault Pipe)
```python
class EvidenceSignature(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4)
    document = models.ForeignKey(RawDocument, on_delete=models.CASCADE)
    signer_did = models.CharField(max_length=512)  # OIDC sub claim
    signature = models.TextField()                  # Tauri bridge response
...
- OIDC Sovereign Auth (mozilla-django-oidc, MyOIDCAuthenticationBackend)
- Session Isolation (hiver_sessionid / hiver_csrftoken)
- Unified Mesh Header (_nav.html + SovereignHeader.tsx with :9001 probe)
- EvidenceSignature model (Vault Pipe / sign_evidence)
- Auth Gate alignment (redirect unauthenticated → OIDC init)
...
- Frontend JS: `index-I83D9tTN.js`
- Frontend CSS: `index-BY2PQJFe.css`
- Vite Manifest: `static/frontend/.vite/manifest.json`

**Last Updated**: 2026-05-15
**Version**: 2.1 (Sovereign Mesh Integration)
