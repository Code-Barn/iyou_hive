...
const EventCard: React.FC<EventCardProps> = ({
  ...
  return (
    <div className={`border rounded-lg p-4 ${statusColor} ${hasReplacesEvent ? 'ring-2 ring-yellow-500' : ''}`}>
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-2">
          <span className={`px-2 py-1 text-xs text-white rounded ${partyColor}`}>
            {event.source_party}
          </span>
      ...
      {event.evidence.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-2">
          ...
          {event.evidence.length > 3 && (
            <span className="text-xs text-gray-500">+{event.evidence.length - 3} more</span>
          )}
        </div>
      )}
    </div>
